"""Per-queue operational command center — v1.9.
...
def queue_command_center(queue_name: str) -> dict[str, Any]:
    ...
    with sync_session_factory() as db:
        ...
        silent = InactivityEngine.detect_silent_breaches(
            db, min_inactivity_ratio=0.5, limit=20, queues=[queue_name],
        )

        # ── 10. Loss attribution share (reuse sla_loss_engine) ─────────
        all_top = SLALossEngine.top_loss_queues(db, limit=200, queues=[queue_name])
